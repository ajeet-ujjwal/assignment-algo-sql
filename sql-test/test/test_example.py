...
class TestExample(DbTest):
    ...
    def test_count_the_number_of_subordinates(self, conn):
        ...
        sql = """
        Select count(B.customer_organization_id) as subordinates_count, A.id as id from organizations A left join enterprise_sales_enterprise_customers B on A.id = B.sales_organization_id left join organizations C on C.id = B.customer_organization_id GROUP BY A.id ORDER BY A.id;
        """
        with conn.cursor(cursor_factory=RealDictCursor) as cur:
            cur.execute(sql)
            actual = cur.fetchall()
    ...
    def test_segments_using_geojson_boundary(self, conn):
        ...
        sql = """
        SELECT id from japan_segments j where ST_contains(ST_GeomFromText(ST_AsText(ST_GeomFromGeoJSON('{"type":"Polygon","coordinates":[[[130.27313232421875,30.519681272749402],[131.02020263671875,30.519681272749402],[131.02020263671875,30.80909017893796],[130.27313232421875,30.80909017893796],[130.27313232421875,30.519681272749402]]]}')), 4326), j.bounds);
        """
        with conn.cursor(cursor_factory=RealDictCursor) as cur:
            cur.execute(sql)
            actual = cur.fetchall()
